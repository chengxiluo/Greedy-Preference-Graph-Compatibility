    ...
    with open(filename, encoding='utf-8', mode = 'r') as f:
        for line in f:
            ...
            if currentTopic == 'Query':
                continue
            for i in [jObj[3], jObj[4], jObj[5]]:
                if i in ['-1','-2']:
                    jud[currentTopic] = jud.setdefault(currentTopic, graph()).add_edge(jObj[1], jObj[2])
                elif i in ['1','2']:
                    jud[currentTopic] = jud.setdefault(currentTopic, graph()).add_edge(jObj[2], jObj[1])
